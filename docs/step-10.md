...
`GET /api/ai/status`는 `{ "available": false }` 또는 `{ "available": true }`를 반환합니다. `available`은 **서버에 비어 있지 않은 API 키가 설정되어 있는지**를 뜻합니다. 키 유효성, 모델 접근 권한, 잔액이나 실제 연결 성공 여부를 미리 검사하지 않으며, 이 조회로 외부 API를 호출하지 않습니다.
...
응답은 `{reply, source, recommendedRecipes}`입니다. `recommendedRecipes`는 서버가 계산한 [추천 응답](api.md) 최대 3개이며, 모델이 생성한 임의 레시피 데이터로 덮어쓰지 않습니다. 활용 가능한 재료가 없으면 빈 배열일 수 있습니다.
...
키는 백엔드 환경 변수 `OPENAI_API_KEY`에서 읽습니다. 기본 모델은 `gpt-4.1-mini`이며 `OPENAI_MODEL`로 변경할 수 있습니다. 기본 서버 주소는 `https://api.openai.com`이고 `OPENAI_BASE_URL`로 바꿀 수 있습니다. 주소 끝에 `/v1`이 있는 형식도 지원합니다.

서버는 `POST /v1/responses`에 `model`, `instructions`, `input`, `max_output_tokens: 1600`, `store: false`를 보냅니다. `store: false`는 응답 저장을 사용하지 않도록 요청하는 설정이며, 모든 서비스 로그나 보관 정책이 없어짐을 뜻하지는 않습니다. 연동 형식과 여러 출력 항목 처리 방법은 [OpenAI 공식 텍스트 생성 문서](https://developers.openai.com/api/docs/guides/text)를 참고했습니다.
...
- 서버가 선택한 추천 최대 3개의 재료 수량·부족량·조리 순서·영양 추정값 등 추천 데이터
- 검증된 이전 대화와 현재 질문
...
HTTP 연결에는 10초, 요청에는 기본 35초 제한을 둡니다. 요청 제한은 `app.openai.timeout-seconds` 설정으로 조정합니다. 리다이렉트는 따라가지 않습니다.
...
| 상황 | 오류 코드 |
| --- | --- |
| OpenAI 401·403 | `AI_CONFIGURATION_ERROR` |
| OpenAI 429 | `AI_RATE_LIMITED` |
| 요청 시간 초과 | `AI_TIMEOUT` |
...
서버는 외부 오류 본문이나 인증 헤더를 사용자에게 그대로 노출하지 않습니다. AI 실패를 성공한 답변으로 표시하거나 자동으로 LOCAL 답변으로 바꾸지 않습니다. 화면에서 오류와 재시도 버튼을 제공하며, 일반 레시피 추천은 계속 이용할 수 있습니다.
...
```powershell
.\mvnw.cmd --batch-mode --no-transfer-progress '-Dtest=ChatServiceTest,OpenAiClientTest,ChatIntegrationTest' test
```

채팅 테스트 23개가 통과했습니다. 통합 테스트는 H2 MySQL 모드와 실제 보안 필터를 사용하고, OpenAI 테스트는 localhost HTTP 서버 및 모의 시간 초과를 사용합니다. **실제 OpenAI 키로 유료 API를 호출하는 검증은 수행하지 않았습니다.** 프로젝트 전체 검증 결과는 [STEP 11](step-11.md)을 참고하세요.
