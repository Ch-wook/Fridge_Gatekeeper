...
export default function AuthPage({ signup, onLogin, notice }) {
  const [error, setError] = useState(null)
  const [busy, setBusy] = useState(false)
  const [registered, setRegistered] = useState(false)
  const isSignup = signup && !registered
  ...
  return <div className="auth-layout">
    ...
    <main className="auth-form-wrap"><div className="auth-card"><p className="eyebrow">WELCOME TO YOUR KITCHEN</p><h2>{isSignup ? '나만의 냉장고 시작하기' : '다시 만나 반가워요'}</h2><p className="muted">{isSignup ? '간단한 가입으로 식재료 관리를 시작하세요.' : '로그인하고 오늘의 냉장고를 확인해 보세요.'}</p>
      {(notice || registered) && <p className="success-box" role="status">{registered ? '가입이 완료되었어요. 입력한 정보로 로그인해 주세요.' : notice}</p>}
      ...
      <form className="stack" onSubmit={submit}>
        ...
        <label className="field">이메일<input name="email" type="email" autoComplete="username" placeholder="you@example.com" required maxLength={254} disabled={busy} /></label>
